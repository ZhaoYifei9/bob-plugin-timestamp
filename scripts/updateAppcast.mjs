...
function updateAppcast(version, desc) {
  const releaseFile = path.join(__dirname, '../release', `timestamp-${version}.bobplugin`)
  ...
  const versionInfo = {
    ...
    desc: desc,
    sha256: fileHash,
    url: `https://github.com/yuzhang9804/bob-plugin-timestamp/releases/download/v${version}/${path.basename(releaseFile)}`,
    minBobVersion: '0.5.0',
  }
  ...
  if (fs.existsSync(appcastFile)) {
    ...
    appcast = { identifier: 'yuzhang.timestamp', versions: [] }
  }
